...
const PILOTO = {
  ...
  // O mesmo Informativo oficial também é servido pelo SCON. O host
  // processo.stj.jus.br recusa os runners do GitHub e, desde 12/09/2026,
  // também a saída do Worker; o SCON entrega a edição corrente com a mesma
  // estrutura HTML, portanto o leitor abaixo não precisa de uma segunda regra.
  stjInfo: 'https://scon.stj.jus.br/jurisprudencia/externo/informativo/?aplicacao=informativo',
...
function completarTesesSTF_(registros){
 ...
 if(lote.length){
  ...
  respostas.forEach((resposta,i)=>{
   ...
   const tese=parseTeseSTF_(resposta.getContentText('ISO-8859-1'),lote[i].numero);
...
function parseTeseSTF_(html,numeroEsperado){
 // A página tem uma linha de dados com Tema | Leading Case | Tese. O cabeçalho
 // já usou td e hoje usa th; por isso a linha é lida isoladamente, sem contar
 // células do documento inteiro.
 for(const linha of html.matchAll(/<tr\b[^>]*>([\s\S]*?)<\/tr>/gi)){
  const celulas=[...linha[1].matchAll(/<td\b[^>]*>([\s\S]*?)<\/td>/gi)].map(m=>htmlTexto_(m[1]));
  if(celulas.length<3||!/^\d+$/.test(celulas[0].trim()))continue;
  const numero=String(Number(celulas[0].trim()));
  if(numeroEsperado!=null&&numero!==String(Number(numeroEsperado)))continue;
  return limpaSTF_(celulas[2]);
 }
 return '';
}
/**
