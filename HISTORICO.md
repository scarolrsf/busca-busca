# Busca Busca — histórico

Registro datado das alterações do projeto, da mais recente para a mais antiga.
...
o README e escolheu, entre manter tudo num arquivo, separar o histórico ou só
corrigir erros, separar o histórico.

**Motivo.** O README tinha 2.753 linhas, e 78% delas eram as 44 entradas de
histórico. Quem abria o documento para saber como o portal funciona hoje
precisava atravessar o estado atual sabendo que o fim do arquivo era outra coisa.

**O que mudou.**

- `HISTORICO.md` (novo): as 44 entradas, movidas por script, byte a byte, na
  mesma ordem — nenhuma foi reescrita.
- `README.md`: fica só o estado atual (692 linhas). Ganha índice no topo e
  duas seções: **Conferências**, com os comandos de autoteste e de prévia que
  antes estavam espalhados, e **Pendências em aberto**, consolidada a partir
  das entradas de 12/09/2026. A "Regra permanente de atualização" passa a
  apontar para o HISTORICO.md.
- Correções encontradas na releitura:
  - "Publicar" dizia que `site/dados.json` é versionado pela coleta e que não há
    build. Não é: está no `.gitignore` e o `publicar.yml` o gera. O texto agora
    concorda com "O que é base e o que é derivado".
  - O inventário não listava `.github/workflows/`, `coleta/monitorar.mjs`,
    `coleta/servir.mjs`, `dados/jurisprudencia-stf.json`, `LICENSE`, `AGENTS.md`,
    `CLAUDE.md` nem este arquivo; a indentação da pasta `coleta/` estava
    desalinhada.
- `CLAUDE.md` e `AGENTS.md`: a regra do documento único vira a regra dos dois
  documentos, e "publicada no Google" vira "publicada no GitHub Pages", onde o
  portal está desde 10/09/2026.
- `MIGRACAO.md`: a regra de convivência aponta o histórico para cá.

**Validação.** Contagem de entradas antes e depois: 44 e 44. O README original
não tinha nada depois do histórico, então o corte foi num ponto só. Nenhum
arquivo de código cita o histórico do README; o `coletar-aqui.cmd` cita a seção
"Coleta na máquina da Sarah", que continua no README com o mesmo nome.

**Dados e implantação.** Nada muda na base, na coleta nem no site. Só
documentação.

**Pendências.** A lista de pendências em aberto foi montada com as entradas de
12/09/2026; as de dias anteriores não foram reavaliadas uma a uma. Avisar o
Codex da nova regra dos dois documentos.
